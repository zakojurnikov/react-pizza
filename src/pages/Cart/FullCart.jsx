...
import { cleanCart, updateGlobalDates } from '../../redux/actions/cart';

const FullCart = () => {
	const store = useSelector(({ cart }) => cart);
	const pizzas = store.items;
		...
		<div className="container container--cart">
			<div className="cart">
				...
				<div className="content__items content__items--cart">
					{pizzas && pizzas.map((pizza, index) => <CartBlock key={index} pizza={pizza} index={index} />)}
				</div>
				<div className="cart__bottom">
					<div className="cart__bottom-details">
						<span> Всего пицц: <b>{store.totalCount} шт.</b> </span>
						<span> Сумма заказа: <b>{store.totalPrice} ₽</b> </span>
					</div>
					<div className="cart__bottom-buttons">
						<NavLink to="/" activeClassName="button button--outline button--add go-back-btn">
